...
SyncMediator::SyncMediator(catalog::WritableCatalogManager *catalog_manager,
  ...
  const int retval = pthread_mutex_init(&lock_file_queue_, NULL);
  assert(retval == 0);
...
void SyncMediator::EnsureAllowed(SharedPtr<SyncItem> entry) {
  ...
  const string relative_path = entry->GetRelativePath();
...
void SyncMediator::EnterDirectory(SharedPtr<SyncItem> entry) {
  ...
  const HardlinkGroupMap new_map;
...
void SyncMediator::InsertHardlink(SharedPtr<SyncItem> entry) {
  ...
  const uint64_t inode = entry->GetUnionInode();
...
void SyncMediator::PublishFilesCallback(const upload::SpoolerResult &result) {
  ...
  {
    const MutexLockGuard guard(lock_file_queue_);
...
void SyncMediator::AddUnmaterializedDirectory(SharedPtr<SyncItem> entry) {
  // The caller gates on IsNew(), which answers by stat()ing the rdonly mount.
  // A mountless (gateway) publish has no such mount, so every ancestor looks
  // new and a second publish re-adds it: UNIQUE constraint failed.
  catalog::DirectoryEntry existing;
  if (catalog_manager_->LookupPath("/" + entry->GetRelativePath(),
                                   catalog::kLookupDefault, &existing)
