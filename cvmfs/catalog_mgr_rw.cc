...
WritableCatalogManager::WritableCatalogManager(
  ...
  sync_lock_ = reinterpret_cast<pthread_mutex_t *>(
      smalloc(sizeof(pthread_mutex_t)));
  int retval = pthread_mutex_init(sync_lock_, NULL);
  assert(retval == 0);
  catalog_processing_lock_ = reinterpret_cast<pthread_mutex_t *>(
      smalloc(sizeof(pthread_mutex_t)));
...
manifest::Manifest *WritableCatalogManager::CreateRepository(
  ...
  const string file_path = dir_temp + "/new_root_catalog";

  const shash::Algorithms hash_algorithm = spooler->GetHashAlgorithm();
  ...
  const int64_t catalog_size = GetFileSize(file_path);
  if (catalog_size < 0) {
...
bool WritableCatalogManager::FindCatalog(const string &path,
  ...
  const bool retval = MountSubtree(ps_path, best_fit, true /* is_listable */,
                                   &catalog);
  if (!retval)
    return false;
  ...
  *result = static_cast<WritableCatalog *>(catalog);
  ...
  const bool found = catalog->LookupPath(ps_path, dirent);
...
WritableCatalog *WritableCatalogManager::GetHostingCatalog(
  ...
  const bool retval = FindCatalog(MakeRelativePath(path), &result, NULL);
  if (!retval)
    return NULL;
  return result;
}
...
void WritableCatalogManager::RemoveDirectory(const std::string &path) {
  ...
  if (parent_entry.IsNestedCatalogRoot()) {
    LogCvmfs(kLogCatalog, kLogVerboseMsg, "updating transition point %s",
             parent_path.c_str());
    WritableCatalog *parent_catalog = reinterpret_cast<WritableCatalog *>(
        catalog->parent());
...
bool WritableCatalogManager::Clone(const std::string destination,
  ...
  const bool destination_already_present = LookupPath(
      MakeRelativePath(destination), kLookupDefault, &check_dirent);
...
void WritableCatalogManager::CloneTreeImpl(const PathString &source_dir,
  ...
  const PathString relative_source(MakeRelativePath(source_dir.ToString()));
  ...
  for (unsigned i = 0; i < ls.size(); ++i) {
    ...
    if (ls[i].IsChunkedFile()) {
      ...
      const std::string relative_sub_path = MakeRelativePath(
          sub_path.ToString());
 ...
 */
bool WritableCatalogManager::HasDirectory(
    const std::string &name, const std::string &parent_directory) {
  const string parent_path = MakeRelativePath(parent_directory);
...
void WritableCatalogManager::AddFile(const DirectoryEntry &entry,
  ...
  const unsigned mbytes = entry.size() / (1024 * 1024);
...
void WritableCatalogManager::AddHardlinkGroup(
  ...
  const unsigned mbytes = entries[0].size() / (1024 * 1024);
  if ((file_mbyte_limit_ > 0) && (mbytes > file_mbyte_limit_)) {
    LogCvmfs(kLogCatalog, kLogStderr,
...
void WritableCatalogManager::TouchDirectory(const DirectoryEntryBase &entry,
  ...
  const PathString transition_path(entry_path.data(), entry_path.length());
  ...
  if (potential_transition_point.IsNestedCatalogMountpoint()) {
    LogCvmfs(kLogCatalog, kLogVerboseMsg, "updating transition point at %s",
             entry_path.c_str());
 ...
 * @return true on success, false otherwise
 */
void WritableCatalogManager::CreateNestedCatalog(
  ...
  bool retval = new_catalog_db->InsertInitialValues(
      nested_root_path,
      volatile_content,
  ...
  Catalog *new_catalog = CreateCatalog(ps_nested_root_path, shash::Any(),
                                       old_catalog);
  ...
  const Catalog::NestedCatalogList
      &grand_nested = wr_new_catalog->ListOwnNestedCatalogs();
  DeltaCounters fix_subtree_counters;
  for (Catalog::NestedCatalogList::const_iterator i = grand_nested.begin(),
                                                  iEnd = grand_nested.end();
       i != iEnd;
       ++i) {
  ...
  const DeltaCounters save_counters = wr_new_catalog->delta_counters_;
...
void WritableCatalogManager::SwapNestedCatalog(const string &mountpoint,
  ...
  // Update counters
  const DeltaCounters delta = Counters::Diff(old_counters,
...
bool WritableCatalogManager::TryGraftNestedCatalog(const string &mountpoint,
  ...
  parent_catalog->UpdateEntry(parent_entry, parent_path);
  if (parent_entry.IsNestedCatalogRoot()) {
    WritableCatalog *grand_parent_catalog = reinterpret_cast<WritableCatalog *>(
  ...
  parent_catalog->InsertNestedCatalog(nested_root_path, NULL, new_hash,
                                      new_size);

  // Fix-up counters
  const Counters counters;
  const DeltaCounters delta = Counters::Diff(counters,
                                             new_catalog->GetCounters());
...
bool WritableCatalogManager::SetVOMSAuthz(const std::string &voms_authz) {
  bool result;
  SyncLock();
  result = reinterpret_cast<WritableCatalog *>(GetRootCatalog())
               ->SetVOMSAuthz(voms_authz);
  SyncUnlock();
  return result;
}
...
bool WritableCatalogManager::Commit(const bool stop_for_tweaks,
  ...
  WritableCatalog *root_catalog = reinterpret_cast<WritableCatalog *>(
      GetRootCatalog());
...
WritableCatalogManager::CatalogInfo WritableCatalogManager::SnapshotCatalogs(
  ...
  spooler_->RegisterListener(&WritableCatalogManager::CatalogUploadCallback,
                             this, upload_context);
  ...
  const CatalogInfo &root_catalog_info = root_catalog_info_future.Get();
...
void WritableCatalogManager::FinalizeCatalog(WritableCatalog *catalog,
  ...
  if (catalog->IsRoot()) {
    LogCvmfs(kLogCatalog, kLogVerboseMsg,
             "setting '%s' as previous revision "
             "for root catalog",
  ...
  const uint64_t catalog_limit = uint64_t(1000)
  ...
  if (stop_for_tweaks) {
    LogCvmfs(kLogCatalog, kLogStdout,
             "Allowing for tweaks in %s at %s "
             "(hit return to continue)",
             catalog->database_path().c_str(), catalog->mountpoint().c_str());
    const int read_char = getchar();
...
void WritableCatalogManager::ScheduleCatalogProcessing(
    WritableCatalog *catalog) {
  {
    const MutexLockGuard guard(catalog_processing_lock_);
...
bool WritableCatalogManager::CopyCatalogToLocalCache(
  ...
  FILE *fcatalog = CreateTempFile(dir_cache_ + "/txn/catalog", 0666, "w",
                                  &tmp_catalog_path);
  if (!fcatalog) {
...
void WritableCatalogManager::CatalogUploadCallback(
  ...
  {
    const MutexLockGuard guard(catalog_processing_lock_);
    const std::map<std::string, WritableCatalog *>::iterator
        c = catalog_processing_map_.find(result.local_path);
  ...
  if (catalog->HasParent()) {
    ...
    const int remaining_dirty_children = catalog->GetWritableParent()
...
bool WritableCatalogManager::GetModifiedCatalogLeafsRecursively(
    Catalog *catalog, WritableCatalogList *result) const {
...
int WritableCatalogManager::GetModifiedCatalogsRecursively(
    const Catalog *catalog, WritableCatalogList *result) const {
  ...
  // on the way.
  const WritableCatalog *wr_catalog = static_cast<const WritableCatalog *>(
      catalog);
...
void WritableCatalogManager::CatalogUploadSerializedCallback(
    const upload::SpoolerResult &result, const CatalogUploadContext unused) {
  if (result.return_code != 0) {
    PANIC(kLogStderr, "failed to upload '%s' (retval: %d)",
          result.local_path.c_str(), result.return_code);
  }
...
WritableCatalogManager::SnapshotCatalogsSerialized(const bool stop_for_tweaks) {
...
void WritableCatalogManager::SetupSingleCatalogUploadCallback() {
  spooler_->RegisterListener(
      &WritableCatalogManager::SingleCatalogUploadCallback, this);
}
...
void WritableCatalogManager::ScheduleReadyCatalogs() {
  ...
  for (auto it = pending_catalogs_.begin(); it != pending_catalogs_.end();) {
    if ((*it)->dirty_children() == 0) {
      ...
      LogCvmfs(kLogCatalog, kLogVerboseMsg, "scheduled %s for processing",
               (*it)->mountpoint().c_str());
...
// Callback for uploading a single catalog, similar to CatalogUploadCallback.
// The main difference is that this callback would not trigger processing of the
// parent
void WritableCatalogManager::SingleCatalogUploadCallback(
  ...
  WritableCatalog *catalog = NULL;
  {
    ...
    std::map<std::string, WritableCatalog *>::iterator const
        c = catalog_processing_map_.find(result.local_path);
    assert(c != catalog_processing_map_.end());
    catalog = c->second;
  }
...
void WritableCatalogManager::LoadCatalogs(
    const std::string &base_path, const std::unordered_set<std::string> &dirs) {
  // mount everything up to "base_path" first (this would be our lease_path
  // typically)
  Catalog *base_catalog;
  MountSubtree(PathString(base_path), NULL /* entry_point */,
               true /* is_listable */, &base_catalog);

  // start up the downloader
  CatalogDownloadContext context;
  context.dirs = &dirs;
  catalog_download_pipeline_ = new CatalogDownloadPipeline(
      static_cast<SimpleCatalogManager *>(this));
  catalog_download_pipeline_->RegisterListener(
      &WritableCatalogManager::CatalogDownloadCallback, this, context);
  catalog_download_pipeline_->Spawn();

  Catalog::NestedCatalogList nested_catalogs = base_catalog
                                                   ->ListNestedCatalogs();
  for (auto it = nested_catalogs.begin(); it != nested_catalogs.end(); ++it) {
    ...
    if (dirs.find(mountpoint) != dirs.end()) {
      Catalog *catalog = CreateCatalog(it->mountpoint, it->hash,
                                       NULL /* parent */);
      {
        MutexLockGuard const guard(catalog_download_lock_);
        catalog_download_map_.insert(
            std::make_pair(it->hash.ToString(), catalog));
      }
...
void WritableCatalogManager::CatalogDownloadCallback(
    const CatalogDownloadResult &result, CatalogDownloadContext context) {
  ...
  for (auto it = nested_catalogs.begin(); it != nested_catalogs.end(); ++it) {
    ...
    if (context.dirs->find(it->mountpoint.ToString()) != context.dirs->end()) {
                                             ...
                                             NULL /* parent */);
      {
        MutexLockGuard const guard(catalog_download_lock_);
        catalog_download_map_.insert(
            std::make_pair(it->hash.ToString(), child_catalog));
      }
      catalog_download_pipeline_->Process(it->hash);
    }
  }
