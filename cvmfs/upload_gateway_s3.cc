...
bool GatewayS3Uploader::InitS3Manager() {
  ...
  if (!object_list_path_.empty()) {
    LogCvmfs(kLogUploadS3, kLogStdout, "GatewayS3: opening object list '%s' "
...
void *GatewayS3Uploader::MainCollectResults(void *data) {
  ...
  while (true) {
    ...
    if (uploader->object_list_ != NULL) {
      ...
      const int rv = fprintf(uploader->object_list_, "%s %s %s\n",
                             info->object_key.c_str(), ok ? "ok" : "failed",
                             !ok ? "-" : (was_duplicate ? "present"
                                                        : "created"));
